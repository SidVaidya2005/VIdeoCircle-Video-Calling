...
## What is _not_ in the stack (intentionally)
...
- **No JWT for app auth.** App auth uses opaque hex session tokens stored on the user document. (LiveKit auth _is_ a JWT — they're separate systems; don't conflate them.)
